...
class LBR(Layer):
    def __init__(self, channels):
        ...
        # Dense作为Linear op
        self.net = Sequential([ Dense(channels, use_bias=False), BatchNormalization(), ReLU(),
                                Dense(channels, use_bias=False), BatchNormalization(), ReLU()])
...
class StackTransformerLayer(Layer):
    ...
    def call(self, x):
        ...
        x3 = self.msa3(x2)
        x4 = self.msa4(x3)
        return x4 # self.concat([x1, x2, x3, x4])
